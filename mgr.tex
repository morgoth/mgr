...
\cleardoublepage
\chapter{Wstęp}
Ewolucja aplikacji internetowych kieruje się ku rozwiązywaniu problemów ze
...
szukanego obiektu powinno być łatwiejsze. Mechanizm został oparty o bibliotekę \emph{Acts as taggable
...
\begin{listing}
  ...
  \listingcaption{Metoda obliczająca maksymalną liczbę punktów możliwą do zdobycia dla konkretnego pytania}
...
Ostatnie pole jest opcjonalne, pozwala na wyznaczenie z ilu pytań będzie się składała
próba podejścia do testu. Jeśli jest mniejsza niż liczba wybranych ogółem, to pytania będą
...
\subsection{Egzamin}
Model \emph{Exam} łączy egzamin przygotowany przez nauczyciela (o którym można myśleć jako
o szablonie, który jest taki sam dla wszystkich) z konkretnym studentem, dla którego są
generowane pytania i ich kolejność. Obiekt musi znajdować się w jednym z poniższych stanów:
...
rekordu przez studenta, sprawdzamy czy egzamin nie powinien się zakończyć. Jeśli dostępny czas
został przekroczony do rekordu nie są wprowadzane zmiany (odpowiedź studenta nie zostaje uznana). Kod został
...
jednokrotnego i wielokrotnego wyboru albo wpisany tekst przy pytaniu porównującym do wzorca. Przed
...
Jeśli wartością pola \texttt{value} jest ,,true'' oznacza to, że została ona wybrana przez
studenta. W takim przypadku przepisujemy wartość punktową zdefiniowaną przez nauczyciela, w przeciwnym
...
zwyczajów, takich jak podział dokumentu na sekcje: nagłówek, nawigacja, stopka,
szczególnie że w wersji 5 języka \emph{HTML} będą miały one swoje własne znaczniki,
...
\emph{MIT} \cite{yui-app-theme}. Został on przystosowany do pracy z narzędziami
...
Ponowne wykorzystanie kodu jest zwłaszcza widoczne przy używaniu modułu zaokrąglającego
...
unikalność istotnych atrybutów. Przykład użycia został przedstawiony na listingu nr~\ref{listing:authlogic}.
...
\emph{Acts as list} umożliwia dołączenie do modelu funkcję listy, czyli
...
Analogicznie, przy pytaniach wielokrotnego lub jednokrotnego wyboru jest losowane miejsce
...
Przykład wykorzystania mechanizmu tagowania został przedstawiony na
listingu nr~\ref{listing:tagging-questions}.
...
każdy nauczyciel mógł w wygodny sposób określić dziedzinę tworzonego przez siebie
...
czemu użycie nie różni się zbytnio od dobrze znanych konwencji. Do działania wymaga
...
Sposób użycia został przedstawiony na listingu nr~\ref{listing:paperclip}.
...
mechanizmu bez przejmowania się problemem dodatkowej konfiguracji ścieżek. Działa na zasadzie przechwycenia
...
\begin{listing}
  ...
  \listingcaption{Wykorzystanie gema ułatwiającego konstruowanie formularzy}
...
która jednak nie jest pozycją wymaganą, przez co będzie otoczona inną klasą \emph{CSS}.
...
Maszyna stanowa pozwala wprowadzić do obiektu pojęcie stanu w jakim obecnie się
...
funkcji. Przykładowa implementacja została przedstawiona na listingu nr~\ref{listing:state-machine}.
...
sprawdzamy czy mogą one zaistnieć. Jeśli zwrócą fałsz, obiekty pozostaną w swoim poprzednim stanie.
...
analogiczny mechanizm jest uruchamiany przy zakończeniu testu.\\
...
\begin{figure}[h!t]
  ...
  \end{center}
  \caption{Diagram stanów i przejść egzaminu}
...
Erik Veenstra \cite{veenstra}. Kod został jednak przystosowany do polskich znaków oraz
...
niedogodności. We frameworku \emph{Ruby on Rails} zostały wprowadzone metody, które eliminowały
...
liczbą bajtów w jakiej jest zapisany dany znak, jednak pojawiają się nowe, dotyczące
...
Instalowany jest on do katalogu domowego użytkownika, a każda wersja posiada swój zestaw gemów.

\cleardoublepage
\chapter{Podsumowanie}
W dzisiejszych czasach na popularności zyskują projekty tworzone na wolnych licencjach,
...
dostosowania kodu do indywidualnych potrzeb, co sprawia że wszelkie dostępne
...
7405 ogólnodostępnych gemach.


Niniejsza praca również została stworzona na wolnej licencji.
...
powinny być trudne. Wszystkie modele, odpowiedzialne za główną logikę aplikacji, są pokryte
